...
namespace SayMoreTests.Utilities
{
	...
	public class SessionArchivingTests
	{
		...
		public void AddIMDISession_RecordingEquipment_EquipmentInKeysOfModelIMDISession()
		{
			...
			AddIMDISessionTestVerify(model, imdiSession, sourceMediaFile1);
		}
		...
		public void AddIMDISession_SetEthnicGroup_PersonHasEthnicGroup()
		{
			...
			AddIMDISessionTestVerify(model, imdiSession);
		}

		public void AddIMDISession_AddSituation_AddSituationAsContentKey()
		{
			const string sampleSituation = "village hut recording";
			_session._metaFile.Setup(f => f.GetStringValue("situation", It.IsAny<string>())).Returns(sampleSituation);
			var model = AddIMDISessionTestSetup(out var imdiSession);
			ArchivingHelper.AddIMDISession(_session, model.Object);
			var situationValue = (from k in imdiSession.Object.MDGroup.Content.Keys.Key
								  where k.Name == "Situation"       // They keyword in the saymore session is lower case but in Imdi we chose upper case
		...
		public void AddIMDISession_SessionCustomField_AddCustomKeyValueAsContentKeyValue()
		{
			...
			_session._metaFile.Setup(f => f.GetCustomFields()).Returns(new[] {new FieldInstance(XmlFileSerializer.kCustomFieldIdPrefix + sampleKey, sampleValue)});
			var model = AddIMDISessionTestSetup(out var imdiSession);
			ArchivingHelper.AddIMDISession(_session, model.Object);
		...
		public void AddIMDISession_AddNotes_AddContentDescription()
		{
			...
			_session._metaFile.Setup(f => f.GetStringValue("notes", It.IsAny<string>())).Returns(sampleNote);
			var model = AddIMDISessionTestSetup(out var imdiSession);
			ArchivingHelper.AddIMDISession(_session, model.Object);
		...
		public void AddIMDISession_ExportSessionContributors_SessionContributorsIncluded()
		{
			...
			contributionCollection.VerifyAll();
			AddIMDISessionTestVerify(model, imdiSession, sourceMediaFile1);
		}
		...
		private Mock<IMDIArchivingDlgViewModel> AddIMDISessionTestSetup(out Mock<SIL.Archiving.IMDI.Schema.Session> imdiSession, ComponentFile mediaFile = null)
		{
			...
			_session._mediaFiles = mediaFile == null ? new ComponentFile[0] : new[] { mediaFile };
			return model;
		}
		...
		private static void AddIMDISessionTestVerify(Mock<IMDIArchivingDlgViewModel> model, Mock<SIL.Archiving.IMDI.Schema.Session> imdiSession, Mock<ComponentFile> sourceMediaFile1 = null)
		{
			model.VerifyAll();
			imdiSession.VerifyAll();
			sourceMediaFile1?.VerifyAll();
		}
		#endregion
	}
	...
	public class DummySession : Session
	{
		...
		public readonly Mock<ProjectElementComponentFile> _metaFile = new Mock<ProjectElementComponentFile>();
